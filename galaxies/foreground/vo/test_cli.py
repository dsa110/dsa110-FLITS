...
def test_run_catalog_smoke(tmp_path, monkeypatch, fake_glade2_rows):
    ...
    def fake_cone_query(*args, **kwargs):
        return fake_glade2_rows.copy(), _fake_meta(args[4], args[5], args[6], kwargs.get("maxrec", 200))

    monkeypatch.setattr(cli, "cone_query", fake_cone_query)

    output = tmp_path / "candidates.csv"
    summary = tmp_path / "summary.csv"
    cli.main(["run-catalog", str(targets_yaml), "glade2",
    ...
    expected_cols = {
        "name", "id", "ra", "dec", "z", "z_type", "richness", "m_delta",
        "r_delta", "delta_def", "service", "table", "provenance_json",
        "theta_arcmin", "b_kpc", "r_delta_computed", "rank_key", "b_over_rvir",
        "intersects_nominal", "intersects_loose", "intersects_strict",
        "is_foreground", "frb_name", "frb_z",
    }
...
def test_host_dz_tolerance_split_by_z_type(tmp_path, monkeypatch):
    ...
    output = tmp_path / "candidates.csv"
    summary = tmp_path / "summary.csv"
    cli.main(["run-catalog", str(targets_yaml), "glade-plus",
              "--output", str(output), "--summary-output", str(summary)])
...
@pytest.mark.unit
def test_run_catalog_warning_lines_match_final_summary_status(tmp_path, monkeypatch, capsys, fake_glade2_rows):
    ...
    out = capsys.readouterr().out
    no_cand_section = out.split("no_candidates_in_cone")[1] if "no_candidates_in_cone" in out else ""
...
@pytest.mark.unit
def test_run_catalog_marks_z_frb_unknown_when_redshift_omitted(tmp_path, monkeypatch, capsys, fake_glade2_rows):
...
@pytest.mark.unit
def test_run_catalog_marks_possibly_truncated_sightlines(tmp_path, monkeypatch, capsys, fake_glade2_rows):
    ...
    summary = tmp_path / "summary.csv"
    cli.main(["run-catalog", str(targets_yaml), "glade2",
              "--output", str(output), "--summary-output", str(summary), "--maxrec", "2"])
...
def test_plan_queries_writes_maxrec_column(tmp_path):
    ...
    cli.main(["plan-queries", str(targets_yaml), "--solver", "greedy",
              "--budget", "5", "--maxrec", "9000", "--output", str(output)])
