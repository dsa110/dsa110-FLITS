...
def _toy_model() -> tuple[FRBModel, list[FRBParams]]:
    ...
    params = [
        FRBParams(c0=1.0, t0=-0.22, gamma=0.0, zeta=0.16, tau_1ghz=0.08, alpha=3.5),
        FRBParams(c0=1.0, t0=0.31, gamma=0.0, zeta=0.13, tau_1ghz=0.08, alpha=3.5),
    ]
    data = (
...
def _brute_force_gain_evidence(
    ...
    kernels = np.stack([
        model(p, "M3", freq_subset=model.valid)
        for p in params
    ])
...
def test_gain_marginal_multi_band_is_label_swap_invariant():
    ...
    swapped, _ = _gain_marginal_multi_band(
        model, list(reversed(params)), ["M3", "M3"], s2=1.3
    )
